...
void setup() {
 ...
 font1= createFont("Helvetica", 7);
  ...
  global1D = loaddata("./points/LdisMatrixLocal.txt", 5, 361);
  ...
  similarity = new TimeSliceSimilarity(SimiX, SimiY, SimiWidth, SimiHeight, day, week, month, season, transpose(RawData), xLabels);

  themeRiver = new InteractThemeRiver((float)ThemeRiverX, (float)ThemeRiverY, (float)ThemeRiverWidth, (float)ThemeRiverHeight, model, similarity.getHighlighted());
